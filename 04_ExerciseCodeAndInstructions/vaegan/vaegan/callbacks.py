...
class GenerateImages(Callback):
    def __init__(
        self, output_dir, model, cmap="gray", n_generated_images=10, n_latent_dims=8
    ):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        fig, ax = plt.subplots(
            1, self.n_generated_images, figsize=(self.n_generated_images, 1)
        )
        ...
        # Save to PNG file
        fakes_save_path = os.path.join(
            self.output_dir, f"epoch{epoch + 1:03d}_fakes.png"
        )
...
class SaveImages(Callback):
    def __init__(
        self, output_dir, model, example_images, n_generated_images=10, n_latent_dims=8
    ):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        recon_save_path = os.path.join(
            self.output_dir, f"epoch{epoch + 1:03d}_recons.png"
        )
