...
class Generator(tf.keras.Model):
    def __init__(self, image_shape=(32, 32, 1), name="generator", **kwargs):
        ...
        n_first_layer_filters = (
            128  # Number of filters/channels in the input to first convolutional layer
        )
        n_strided_layers = 2
        first_conv_input_shape = (
            self.image_shape[0] // (2**n_strided_layers),
        ...
        self.tconv0 = tkl.Conv2DTranspose(
            128, 4, strides=(2, 2), padding="same", name="conv0"
        )
        self.relu0 = tkl.LeakyReLU(name="relu0")
        # (d/2) x (d/2)

        self.tconv1 = tkl.Conv2DTranspose(
            128, 4, strides=(2, 2), padding="same", name="conv1"
        )
...
class GAN(tf.keras.Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.generator = Generator(image_shape=self.image_shape)
        ...
        self.optimizer_gen = tf.keras.optimizers.Adam(learning_rate=self.generator_lr)
    ...
    def call(self, inputs, training=None):
        ...
        latent_sample = tf.random.normal((n, self.n_latent_dims))
        # Generate fake images
        return self.generator.call(latent_sample, training=training)
    ...
    def metrics(self):
        ...
        """
        return [self.loss_gen_tracker, self.loss_disc_tracker]
    ...
    def train_step(self, images_real,):
        ...
        latent_sample = tf.random.normal((n, self.n_latent_dims))
        images_fake = self.generator.call(latent_sample, training=False)
        ...
        with tf.GradientTape() as gt1:
            labels_pred = self.discriminator.call(images_disc, training=True)
            disc_loss = self.loss_bce(labels_disc, labels_pred)

        grads_disc = gt1.gradient(disc_loss, self.discriminator.trainable_weights)
        self.optimizer_disc.build(self.discriminator.trainable_weights)
        ...
        with tf.GradientTape() as gt2:
            latent_sample = tf.random.normal((2 * n, self.n_latent_dims))
            images_fake = self.generator.call(latent_sample, training=True)
            labels_pred = self.discriminator.call(images_fake, training=False)
        ...
        self.optimizer_gen.build(self.generator.trainable_weights)
        self.optimizer_gen.apply_gradients(
            zip(grads_gen, self.generator.trainable_weights)
        )

        # 16. Update the running means of the losses
...
class MultiTaskDiscriminator(Discriminator):
    def __init__(self, n_classes, name="discriminator", **kwargs):
...
class ConditionalGAN(GAN):
    ...
    def __init__(self, n_classes=10, cond_loss_weight=1.0, name="cgan", **kwargs):
        """Generative adversarial network containing a generator to synthesize
        images and an adversary to to discriminate between real and fake images.
        ...
        self.loss_class = tf.keras.losses.CategoricalCrossentropy(name="Cat_x_entropy")

        # Classification accuracy
        self.metric_class = tf.keras.metrics.TopKCategoricalAccuracy(
            k=1, name="Cat_accuracy"
        )
    ...
    def call(self, inputs, training=None):
        ...
        _, classes = inputs
        n_samp = tf.shape(classes)[0]
        z_rand = tf.random.normal((n_samp, self.n_latent_dims))
        z_rand_cond = tf.concat([z_rand, classes], axis=1)
        return self.generator(z_rand_cond, training=training)
    ...
    def metrics(self):
        ...
        Return a list of losses
        """
        return [self.loss_gen_tracker, self.loss_disc_tracker, self.metric_class]

    def generate_random_classes(self, n_samples):
        """Generate random one-hot encoded class labels.
        
        Args:
            n_samples: Integer or scalar tensor indicating number of samples to generate
            
        Returns:
            Tensor of shape [n_samples, n_classes] with one-hot encoded class labels
        """
        # Ensure n_samples is a scalar if it's a tensor
        if isinstance(n_samples, tf.Tensor):
            n_samples = tf.cast(n_samples, tf.int32)
        
        # Generate random class indices with equal probability
        class_probs = tf.ones((1, self.n_classes))
        class_indices = tf.random.categorical(class_probs, num_samples=n_samples)
        
        class_indices = tf.reshape(class_indices, [-1])
        
        # Convert to one-hot with explicit shape
        one_hot_classes = tf.one_hot(class_indices, depth=self.n_classes, dtype=tf.float32)
