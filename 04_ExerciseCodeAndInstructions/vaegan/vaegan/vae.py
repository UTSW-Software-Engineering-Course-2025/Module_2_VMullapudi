...
def sample_from_normal(dist_mean, dist_logvar):
    ...
    return dist_mean + tf.exp(dist_logvar / 2) * tf.random.normal(
        shape=tf.shape(dist_mean)
    )
...
def kl_divergence(dist_mean, dist_logvar):
    ...
    return tf.reduce_mean(-0.5 * tf.reduce_sum(
        1 + dist_logvar - tf.square(dist_mean) - tf.exp(dist_logvar), axis=-1
    ))


class Encoder(tf.keras.Model):
    def __init__(self, n_latent_dims=8, name="encoder", **kwargs):
        ...
        # up.
        self.dense_logvar = tkl.Dense(
            self.n_latent_dims, kernel_initializer="zeros", name="dense_logvar"
        )
...
class Decoder(tf.keras.Model):
    def __init__(self, image_shape=(32, 32, 1), name="decoder", **kwargs):
        ...
        n_encoder_last_filters = (
            64  # Number of filters in the encoder's last convolutional layer
        )
        n_encoder_strided_layers = 3
        first_conv_input_shape = (
            self.image_shape[0] // (2**n_encoder_strided_layers),
        ...
        self.tconv0 = tkl.Conv2DTranspose(
            64, 4, strides=(2, 2), padding="same", name="tconv0"
        )
        self.bn0 = tkl.BatchNormalization(name="bn0")
        self.relu0 = tkl.ReLU(name="relu0")
        # (d/4) x (d/4)
        ...
        self.tconv1 = tkl.Conv2DTranspose(
            64, 4, strides=(2, 2), padding="same", name="tconv1"
        )
        self.bn1 = tkl.BatchNormalization(name="bn1")
        self.relu1 = tkl.ReLU(name="relu1")
        # (d/2) x (d/2)

        self.tconv2 = tkl.Conv2DTranspose(
            32, 4, strides=(2, 2), padding="same", name="tconv2"
        )
        self.bn2 = tkl.BatchNormalization(name="bn2")
        self.relu2 = tkl.ReLU(name="relu2")
        # d x d
    ...
    def compute_output_shape(self, input_shape):
        """Compute the output shape of the model given the input shape."""
...
class VAE(tf.keras.Model):
    ...
    def call(self, inputs, training=None):
        ...
        # Add some noise to the encoded representation, so that the position
        # samples a point from a distribution defined by the mean and log-variance.
        z = sample_from_normal(z_mean, z_logvar)

        # Decode the probabilistic latent representation back into an image
    ...
    def metrics(self):
        ...
        """
        return [self.loss_recon_tracker, self.loss_kl_tracker, self.loss_total_tracker]
    ...
    def train_step(self, data):
        ...
        #         unpack the outputs into z_mean and z_logvar
            z_mean, z_logvar = self.encoder.call(images_real, training=True)
        ...
        #        store the result in z
            z = sample_from_normal(z_mean, z_logvar)
        #      4. Now Use decoder to reconstruct image by invoking its forward pass, call, but do that indirectly through self.decoder
        ...
        #         store the result in the  variable recons
            recons = self.decoder.call(z, training=True)
        ...
        # user.
        self.optimizer.build(self.trainable_weights)  # Ensure optimizer is built
...
class ConditionalVAE(VAE):
    ...
    def make_conditional_input(self, images, labels):
        ...
        labels_reshaped = tf.reshape(labels, [batch_size, 1, 1, -1])
        labels_tiled = tf.tile(labels_reshaped, [1, height, width, 1])
    
        # 3. Finally since the tensors are the same dimension along the last axis, we can concatenate them together.
        return tf.concat([images, labels_tiled], axis=-1)
    ...
    def train_step(self, data):
        ...
        # Use encoder to predict probabilistic latent representations
            z_mean, z_logvar = self.encoder.call(encoder_inputs, training=True)
            ...
            recons = self.decoder.call(decoder_inputs, training=True)
